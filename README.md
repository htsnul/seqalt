...
```
値0 関数0 値1 関数1 値2 関数2 ...
```

と、値と関数が交互に並んでいるとみなし、1個おきに関数呼び出しをしていく。
...
また、アローネイティブ関数実行時の環境 `env` を保持している。
...
これは、条件分岐やラムダのような場合に、右側のエクスプレッションを評価したくない場合があるためだ。
...
引数は `args` として参照できるが、
右引数に限って、引数名としてアローネイティブ関数実行時に保持された値があれば、
それらの値でも参照できるように変数追加している。

保持しておいたアローネイティブ関数実行時の環境を `_parentEnv` として設定している。
